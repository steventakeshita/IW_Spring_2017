...
# run the SMT solver on a board to find n sets and return the time it took
def runSMTNoCondensesolver(randomizer, n):
	# checks CPU time... NOT real time
	start = time.clock()
	
	test = SMTsolverNoCondense(randomizer.v, randomizer.p, randomizer)
	model = test.find_n_sets(n)
		...
		for p in properties:
			...
			for n in sets_to_find:
				...
				for _ in range(num_trials):
					...
					Dynamic_time.append(runSMTNoCondensesolver(Dynamic_rand, n))
				...
				print "Value: " + str(v) + " | Properties: " + str(p) + " | Sets found: " + str(n) + " | SMT time: " + str(avg_SMT) + " | No Condense time: " + str(avg_Dynamic) + " | Sorted SMT time: " + str(avg_Brute)  
