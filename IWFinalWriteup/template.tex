...
The satisfying assignment can be achieved through any permutation of cards of a set. Therefore, the SMT solver will be forced to check every permutation of cards for a set, when in reality, all permutations of a given set are equivalent because order does not matter in a set of cards. This will break the symmetry in the encoding and speed up the process of the SMT solver. Therefore, we need to encode this relation of forcing a single set for all permutations of cards. Therefore, we can arbitrarily set that the sum of the values of the cards for all properties must be in ascending order. A set like $\{[000],[001],[002]\}$ will not be allowed to take any other configuration as $0 \leq 1 \leq 2$. This can be encoding as follows: 

\begin{align}
	\bigwedge \limits_{i=1}^{v-1} \left( \sum \limits_{m=1}^{p} k_{i,m} \leq  \sum \limits_{m=1}^{p} k_{i+1,m} \right)
\end{align}
